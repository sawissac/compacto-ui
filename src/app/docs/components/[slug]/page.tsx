      ...
      <Section id="installation" title="Installation">
        <div className="flex flex-col gap-3">
          <p className="text-[12px] leading-relaxed text-app-dim">
            No install — ask an AI agent to copy this component from{" "}
            <code className="font-mono">registry.json</code>
            {entry.peer ? (
              <>
                , then <code className="font-mono">pnpm add {entry.peer}</code>
              </>
            ) : null}
            .
          </p>
          <Code>
            {`import { ${(api?.exports ?? [entry.name]).join(", ")} } from "@/components/ui/${entry.slug}";`}
          </Code>
        </div>
      </Section>
